...
myQuery.define("ui/js/turnbook", ["ui/js/swappable", "main/class", "html5/css3"], function ($, swappable, clas, css33, undefined) {
    "use strict"; //启用严格模式
    var turnBook = $.widget("ui.turnBook", {
        ...
        , create: function () {
            ...
            if (opt.positionType == "half") {
                ...
                this.backgound = $({ a: "absolute", b: opt.pageBackgroundColor }, "div")//.initTransform3d()
            ...
            this.message = $({ a: "absolute", t: this.bookHeight / 4, l: this.bookWidth / 4, w: this.bookWidth / 2, h: this.bookHeight / 2 }, "div").replaceClass(opt.messageClass).addHandler("mousedown", function () {
                $(this).hide();
            }).hide();
        ...
        , event: function () {
            ...
            , turnNextHalf = function (index, offsetX) {
                self.setBoxCss(index, { w: $.between(0, pageWidth, pageWidth + offsetX - bookWidth) })
                self.setCss(self.backgound, { w: $.between(0, pageWidth, (bookWidth - offsetX) / 2), l: offsetX, boxShadow: shadow }, { tx: offsetX });
            }
            , turnPreHalf = function (index, offsetX) {
                index && self.setCss(self.backgound, { w: $.between(0, pageWidth, (bookWidth - offsetX) / 2), l: offsetX, boxShadow: shadow }, { tx: offsetX })
            }
            , turnNextWhole = function (index, offsetX) {
                self.setBoxCss(index, { w: $.between(0, pageWidth, pageWidth + offsetX - bookWidth) })
                                .setBoxCss(index + 1, { w: $.between(0, pageWidth, (bookWidth - offsetX) / 2), l: offsetX, boxShadow: shadow }, { tx: offsetX });
            }
            , turnPreWhole = function (index, offsetX) {
                self.setBoxCss(index - 1, { w: $.between(0, pageWidth, pageWidth - offsetX), l: offsetX }, { tx: offsetX })
                .setPageCss(index - 1, { l: -offsetX })
                        ...
                        switch (e.direction) {
                            ...
                            case 1:
                                if (opt.bookIndex >= opt.pages.length - 1) {
                                    self.showPages(opt.bookIndex);
                                    opt.positionType == "half" && self.showMessage("已经到最后一页");
                                }
                                else {
                                    if (opt.positionType == "half")
                                        self.showPages(opt.bookIndex + 1);
                                    else
                                        self.isInRight(e.startX) ? self.showPages(opt.bookIndex + 2) : self.showPages(opt.bookIndex);
                                }
                                break;
                            case 3: //向前翻页
                            case 4:
                            case 5:
                                if (opt.bookIndex <= 0) {
                                    self.showPages(opt.bookIndex);
                                    opt.positionType == "half" && self.showMessage("已经到第一页");
                                }
                                else {
                                    if (opt.positionType == "half")
                                        self.showPages(opt.bookIndex - 1);
                                    else
                                        self.isInLeft(e.startX) ? self.showPages(opt.bookIndex - 2) : self.showPages(opt.bookIndex);
                                }
                                break;
                        }
                        e.type = "turnbookstop";
        ...
        , getBox: function (index) {
            var page = this.getPage(index), box = page != undefined ? page.box : undefined;
            return box
        }
        ...
        , inductionCorner: function (x, y) {
            var opt = this.options, result = false;
            ...
            //                    result = true;
            //if (result) {
            if (x <= opt.inductionWidth) { result = true }
            else if (x >= this.bookWidth - opt.inductionWidth) { result = true }
            else { result = false; }
        ...
        , setBook: function (bookName, bookType, bookContent, bookIndex) {
            var opt = this.options, cache = this.cache[bookName];
            if (cache) {
                ...
                if (bookContent) {
                    ...
                    this.cache[opt.bookName] = { pages: opt.pages, contents: opt.contents, bookType: bookType, bookIndex: opt.bookIndex };
                }
            }
            return this;
        }
        , setBoxCss: function (index, css, css3d) {
            var opt = this.options, box = this.getBox(index);
            box && this.setCss(box, css, css3d);
            return this;
        }
        , setContextCss: function (index, css) {
            var opt = this.options, content = this.getContent(index);
            if (content) {
                css = css || {};
        ...
        , setPageCss: function (index, css, css3d) {
            var opt = this.options, page = this.getPage(index);
            if (page) {
                css = css || {};
        ...
        , setCss: function (item, css, css3d) {
            ...
            if (css.boxShadow != undefined) {
                item.css3({ boxShadow: css.boxShadow });
            }
            return this;
        }
        ...
        , showPages: function (index) {
            var opt = this.options, begin, close, len = opt.pages.length - 1, pageWidth = this.pageWidth, bookWidth = this.bookWidth;
            ...
            if (opt.positionType == "half") {
                for (var i = index + 2; i >= index; i--) {
                    this.setContextCss(i).setBoxCss(i, { l: 0, w: pageWidth }, { tx: 0 }).setPageCss(i, { l: 0 }).appendTo(i);
                }
                this.setContextCss(i - 1).setBoxCss(i - 1, { l: -pageWidth, w: pageWidth }, { tx: -pageWidth }).setPageCss(i - 1, { l: 0 }).appendTo(i - 1);

                this.setCss(this.backgound, { l: 0, w: this.pageWidth, h: this.pageHeight, boxShadow: "" }, { tx: this.pageWidth });
                ...
                this.setContextCss(index - 3).setBoxCss(index - 3, { l: 0, w: pageWidth, boxShadow: "" }, { tx: 0 }).setPageCss(index - 3, { l: 0 }).appendTo(index - 3);

                this.setContextCss(index - 1).setBoxCss(index - 1, { l: 0, w: pageWidth, boxShadow: "" }, { tx: 0 }).setPageCss(index - 1, { l: 0 }).appendTo(index - 1);

                this.setContextCss(index + 2).setBoxCss(index + 2, { l: pageWidth, w: pageWidth, boxShadow: "" }, { tx: pageWidth }).setPageCss(index + 2, { l: 0 }).appendTo(index + 2);

                this.setContextCss(index).setBoxCss(index, { l: pageWidth, w: pageWidth, boxShadow: "" }, { tx: pageWidth }).setPageCss(index, { l: 0 }).appendTo(index);

                this.setContextCss(index + 1).setBoxCss(index + 1, { l: bookWidth, w: pageWidth, boxShadow: "" }, { tx: bookWidth }).setPageCss(index + 1, { l: 0 }).appendTo(index + 1)
                this.setContextCss(index - 2).setBoxCss(index - 2, { l: -pageWidth, w: pageWidth, boxShadow: "" }, { tx: -pageWidth }).setPageCss(index - 2, { l: 0 }).appendTo(index - 2)
            }
            this.cache[opt.bookName].bookIndex = opt.bookIndex = index;
        }
        , showMessage: function (msg, autoHide) {
            if (this.options.isShowMessage !== true)
