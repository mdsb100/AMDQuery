...
(function (window) {
    ...
    //加载JS文件
		_fnLoadJS = function (url, sModule, fnError) {
		    ...
		    var module = _modules[sModule], resource = _resource;
    ...
    */
		_ClassModule = function (sModule, aDependencies, fnBody, bDeep) {
		    if (!sModule) { return; }
		    var i, l, sDP, aDP = [], mDP, fnErr = error, sMD, modules = _modules, sFrom;
		    this._amdID = sModule;
		    ...
		    if (!aDependencies || !aDependencies.length) {
		        ...
		        if (l === 1 && aDependencies[0] === sModule) {
		            ...
		            if (!aDP.length) {
		                ...
		                sFrom = (modules[sModule] && modules[sModule]._amdFromRequire) ? 'require' : 'define';
		                modules[sModule] = this;
		                if (bDeep) {//深入加载依赖模块
		                    _loadDependencies(aDP, sFrom);
		                }
		            }
		        }
		    }
    ...
    * @optional {String} sPath 指定JS模块名对应的JS文件路径		* @option {Boolean} bNamed 指定该模块是否为具名模块
    ...
    */
	config = function (sNamespace, sPath, bNamed) {
	    ...
	    if (sPath) {
	        ...
	        oMaps[sNamespace] = sPath; if (bNamed) { define.amd.namedModules[sNamespace] = true; }
	    } else {
